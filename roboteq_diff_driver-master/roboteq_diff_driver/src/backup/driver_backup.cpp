...
ROS_DEBUG_STREAM("cmdvel power right: " << right_power << " left: " << left_power);
  ...
  {
    // motor speed (rpm)
    int32_t right_rpm = right_speed / wheel_circumference * 60.0 * gear_ratio;
    int32_t left_rpm = left_speed / wheel_circumference * 60.0 * gear_ratio;
#ifdef _CMDVEL_DEBUG
...
//ROS_DEBUG_STREAM( "line: " << odom_buf );
          ...
          if ( odom_buf[0] == 'C' && odom_buf[1] == 'R' && odom_buf[2] == '=' )
          {
            ...
            for ( delim = 3; delim < odom_idx; delim++ )
            {
              ...
              if (odom_buf[delim] == ':')
              {
                // std::cout << "Hello, " << odom_buf << "\n";
                odom_buf[delim] = 0;
                // std::cout << "Hello, " << odom_buf+3 << "|" << odom_buf+delim+1 << "\n";
