...
public:
  ...
  // void cmdmotor_callback( const std_msgs::String& cmdmotor_msg);
  void cmdvel_setup();
  void cmdvel_loop();
  void cmdvel_run();
  ...
  max_rpm(0)
{
  ...
  // nhLocal.param<std::string>("cmdmotor_topic", cmdmotor_topic, "motor_break");
  // ROS_INFO_STREAM("cmdmotor_topic: " << cmdmotor_topic);
...
// void MainNode::cmdmotor_callback( const std_msgs::String& cmdmotor_msg)
// {
//   controller.write(cmdmotor_msg.data.str());
//   controller.flush();
// }
...
void MainNode::cmdvel_setup()
{
  ...
  // ROS_INFO_STREAM("Subscribing to topic " << cmdmotor_topic);
  // cmdmotor_sub = nh.subscribe(cmdmotor_topic, 3, &MainNode::cmdmotor_callback, this);
...
//    odom_broadcaster.init(nh);	// ???
  ...
  voltage_pub = nh.advertise<std_msgs::Float32>("roboteq/voltage", 3);
  ROS_INFO("Publishing to topic roboteq/current");
  current_pub = nh.advertise<roboteq_diff_msgs::Duplex>("roboteq/current", 3);
  ROS_INFO("Publishing to topic roboteq/energy");
  energy_pub = nh.advertise<std_msgs::Float32>("roboteq/energy", 3);
  ROS_INFO("Publishing to topic roboteq/temperature");
  temperature_pub = nh.advertise<std_msgs::Float32>("roboteq/temperature", 3);
#endif
...
void MainNode::odom_loop()
{
  ...
  if (controller.available())
  {
    ...
    char ch = 0;
    while(DELTAT(nowtime,odom_start_time) < 100)
    {
      // ROS_INFO("Looping");
      nowtime = millis();
      ch = 0;
