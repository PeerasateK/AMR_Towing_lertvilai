...
class map_navigation():
    ...
    def __init__(self):
        ...
        rospy.Subscriber("/amr200/joint_states", JointState, self.joint_state_callback)
    ...
    def find_pose_park_cart(self,xPark,yPark,rzPark):
        ...
        homo_w_amr = np.array([[np.cos(yaw_amr), -np.sin(yaw_amr), 0,self.pose_amr[0]],
                    ...
                    [0, 0, 1,0],
                    [0,0,0,1]])
        homo_amr_cart = np.array([[np.cos(self.pos_hook), -np.sin(self.pos_hook), 0,-1.826*cos(self.pos_hook)],
                    [np.sin(self.pos_hook), np.cos(self.pos_hook), 0,-1.826*sin(self.pos_hook)],
                    [0, 0, 1,0],
                    [0,0,0,1]])
    ...
    def ReversePark(self,xPark,yPark,rzPark):
        ...
        while self.parkState == 3:
            ...
            self.move_cmd.angular.z = speed/1.826
            self.cmd_vel.publish(self.move_cmd)
            print(rad_park_cart,speed,speed/1.826)
